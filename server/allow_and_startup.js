...
Meteor.publish('neuron_catalog_config', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return NeuronCatalogConfig.find({}) }
})

Meteor.publish('driver_lines', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return DriverLines.find({}) }
})
Meteor.publish('neuron_types', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return NeuronTypes.find({}) }
})
Meteor.publish('brain_regions', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return BrainRegions.find({}) }
})
Meteor.publish('binary_data', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return BinaryData.find({}) }
})

Meteor.publish('archive_filestore', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return ArchiveFileStore.find({}) }
})
Meteor.publish('cache_filestore', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return CacheFileStore.find({}) }
})
Meteor.publish('zip_filestore', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) { return ZipFileStore.find({}) }
})

// ----------------------------------------

Meteor.publish('userData', function () {
  if (NeuronCatalogApp.checkRole(this.userId, ReaderRoles)) {
...
let logged_in_allow = {
  insert (userId, doc) {
    return NeuronCatalogApp.checkRole(userId, WriterRoles)
  },
...
ArchiveFileStore.allow({
  ...
  download (userId, fileObj) {
    return NeuronCatalogApp.checkRole(userId, ReaderRoles)
  }
})
...
NeuronCatalogConfig.allow({
  insert (userId, doc) {
    return NeuronCatalogApp.checkRole(userId, ['admin'])
  },
