...
export function convertToPlain (coll, collection_name) {
  // Convert from Meteor.Collection into plain javascript object.
  const this_coll = {}
...
export function export_data () {
  let collections = {}
  let iterable = ['NeuronCatalogConfig', 'DriverLines', 'NeuronTypes',
    'BrainRegions', 'BinaryData', 'Meteor.users', 'SettingsToClient']
  for (let i = 0; i < iterable.length; i++) {
    let collection_name = iterable[i]
    let coll = get_collection_from_name(collection_name)
    const plainObject = convertToPlain(coll, collection_name)
    collections[collection_name] = plainObject
  }
